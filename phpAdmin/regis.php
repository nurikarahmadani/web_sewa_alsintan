...
if (isset($_POST['daftar'])) {
    ...
    $confirpass = htmlspecialchars($_POST['Cpassword']);
    $no_hp = $_POST['hp'];
    $hasil_regis;
    if ($password === $confirpass) {
        $password = password_hash($password, PASSWORD_DEFAULT);
        $select_email = "SELECT email_admin FROM admin WHERE email_admin = '$email'";
        $cek_user = mysqli_query($conn, $select_email);
        if (mysqli_fetch_assoc($cek_user)) {
            $hasil_regis = 'email sudah ada';
        } else {
            $insert_sql = "INSERT INTO admin(nama_admin, pass_admin, noHP_admin, email_admin) VALUES ('$nama', '$password', '$no_hp', '$email')";
